...
import { Sparkles, X, Send, Loader2, ListChecks, MessageCircle, ChevronRight } from 'lucide-react'
...
export function WeaveAssistant({ role, checklist = [] }: WeaveAssistantProps) {
  const { user } = useAuth()
  const router = useRouter()
  const [isOpen, setIsOpen] = useState(false)
  const [tab, setTab] = useState<'checklist' | 'ask'>(checklist.length > 0 ? 'checklist' : 'ask')
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="fixed bottom-24 right-4 sm:bottom-6 sm:right-6 z-50 w-14 h-14 rounded-full flex items-center justify-center hover:scale-105 transition-transform"
        style={{
      ...
      >
        ...
        <div className="flex border-b" style={{ borderColor: 'var(--field-border)' }}>
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 py-2.5 text-xs font-semibold uppercase tracking-wide transition"
            style={{
