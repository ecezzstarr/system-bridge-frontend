...
import { FLAME_EVENT, EventRole, WeaveEvent, getEventProgress, resolveEventStatus } from '@/lib/weave-event'
...
export default function PositionEventWorld({ role, context }: { role: EventRole; context?: { label: string; value: string | number }[] }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/events/flame', { cache: 'no-store' })
      ...
      .catch(() => {
        // The built-in event definition remains the safe fallback.
      })
  ...
  const status = effectiveStatus === 'active' ? 'LIVE' : effectiveStatus === 'planned' ? 'COMING SOON' : 'CLOSED'
  ...
  return <section className="overflow-hidden rounded-3xl border border-white/10 bg-slate-950/90 text-white shadow-2xl">
    ...
    <div className="grid gap-0 lg:grid-cols-[1.2fr_.8fr]">
      <div className="p-5 md:p-7">
        <p className="text-[10px] uppercase tracking-[0.22em] text-sky-300">{position.mode === 'player' ? 'Client Player · Your Event' : `${role === 'admin' ? 'Administration' : 'Support Position'} · Client Movement`}</p>
