...
export function DailyProjectClaim() {
  ...
  const load = async () => {
    ...
    setError('')
    try {
      const response = await fetch('/api/bridger/daily-project', { headers: getAuthHeaders() })
  ...
  const claimToday = async () => {
    ...
    setError('')
    try {
      const response = await fetch('/api/bridger/daily-project', {
        method: 'POST',
  ...
  return (
    <section className="relative overflow-hidden rounded-2xl border border-emerald-500/30 bg-slate-900/80 p-5 sm:p-6 shadow-lg">
      ...
      <div className="relative">
        ...
        {loading && (
          <div className="mt-5 h-20 animate-pulse rounded-xl bg-slate-800/60" />
        )}
