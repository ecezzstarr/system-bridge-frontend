...
CREATE TABLE IF NOT EXISTS chat_rooms (id INTEGER NOT NULL DEFAULT nextval('chat_rooms_id_seq'::regclass), name TEXT NOT NULL, description TEXT, type TEXT NOT NULL DEFAULT 'public'::text, allowed_roles TEXT[], emoji TEXT DEFAULT '💬'::text, color TEXT DEFAULT '#f59e0b'::text, created_at TIMESTAMPTZ NOT NULL DEFAULT now());
...
-- Grant permissions to ssbnow_user
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO ssbnow_user;
GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO ssbnow_user;
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON TABLES TO ssbnow_user;
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON SEQUENCES TO ssbnow_user;
...
CREATE TABLE IF NOT EXISTS subscription_plans (id INTEGER NOT NULL DEFAULT nextval('subscription_plans_id_seq'::regclass), name TEXT NOT NULL, description TEXT, monthly_trx_cost NUMERIC(10,4) NOT NULL, allowed_roles TEXT[] NOT NULL DEFAULT '{}'::text[], created_at TIMESTAMPTZ NOT NULL DEFAULT now());
