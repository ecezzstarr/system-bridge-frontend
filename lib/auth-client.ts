...
export function getToken(): string | null {
  if (typeof window !== 'undefined') {
    try {
      return localStorage.getItem('ssb_auth_token') || (window as any).__auth_token || null
    } catch (e) {
...
export function getUser(): any {
  if (typeof window !== 'undefined') {
    try {
      const user = localStorage.getItem('ssb_auth_user')
...
export function clearToken(): void {
  if (typeof window !== 'undefined') {
    try {
      localStorage.removeItem('ssb_auth_token')
      localStorage.removeItem('ssb_auth_user')
    } catch (e) {
      // Silently fail
    }
