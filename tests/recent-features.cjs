...
assert.ok(weaveDashboardWorldSource.includes('[transform:translateZ(18px)]'),'Operating surface sits forward from the moving world')
...
assert.ok(normalWorldAtmosphereSource.includes('operating-system base'),'Normal WEAVE world remains the operating-system base')
assert.ok(normalWorldAtmosphereSource.includes('<WeaveDashboardWorld'),'Role dashboards keep one persistent WEAVE world')
assert.ok(flameWorldAtmosphereSource.includes('WEAVE System State'),'Active event is expressed as a system state')
assert.ok(!flameWorldAtmosphereSource.includes('FlameEventRiverField'),'Dashboard event state no longer mounts a second event world')
...
assert.ok(flameAdSource.includes('WEAVE SYSTEM STATE'),'Event banner is an operating-system signal')
...
assert.ok(clientDashboardUnifiedSource.includes('<WeaveDashboardWorld'),'Client keeps one persistent world during Company Loops')
assert.ok(clientNavUnifiedSource.includes('Loop 1 Ground'),'Client navigation keeps event access inside the normal shell')
assert.ok(!clientNavUnifiedSource.includes('eventIsLive'),'Client navigation no longer disappears when an event goes live')
...
assert.ok(fs.readFileSync(path.join(root,'components/app-sidebar.tsx'),'utf8').includes('Loop 1 Ground'),'Sidebar exposes Loop 1 as a WEAVE district rather than a separate app')
