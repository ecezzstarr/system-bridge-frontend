    ...
    const clients = q
      ? await sql`
          SELECT c.id, c.name, c.email, c.business_name, c.phone, c.file_number,
                 COALESCE(v.balance, 0) AS vault_balance, COALESCE(v.currency, 'USDT') AS currency
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (action === 'credit') {
      ...
      if (!clientId || !Number.isFinite(value) || value <= 0) return NextResponse.json({ error: 'clientId and positive amount required' }, { status: 400 })
      await sql`INSERT INTO client_vaults (client_id) VALUES (${clientId}::uuid) ON CONFLICT (client_id) DO NOTHING`
      const [vault] = await sql`
        UPDATE client_vaults SET balance = balance + ${value}, updated_at = NOW()
      ...
      await sql`
        ...
        VALUES (${clientId}::uuid, 'credit', ${value}, ${vault.currency}, ${vault.balance}, ${source}, ${reference || null}, ${reason || null}, ${auth.session.user.id}::uuid)
      `
      return NextResponse.json({ success: true, balance: vault.balance, currency: vault.currency })
    }
    ...
    if (action === 'review_withdrawal') {
      ...
      await sql`INSERT INTO client_vaults (client_id) VALUES (${withdrawal.client_id}::uuid) ON CONFLICT (client_id) DO NOTHING`
      const [vault] = await sql`SELECT balance, currency FROM client_vaults WHERE client_id=${withdrawal.client_id}::uuid`
      if (Number(vault.balance) < Number(withdrawal.amount)) return NextResponse.json({ error: 'Insufficient Client Vault balance at approval' }, { status: 409 })
      const [updated] = await sql`
        UPDATE client_vaults SET balance = balance - ${withdrawal.amount}, updated_at=NOW()
        WHERE client_id=${withdrawal.client_id}::uuid RETURNING balance, currency
      `
      await sql`
        INSERT INTO client_vault_ledger (client_id, entry_type, amount, currency, balance_after, source, reference, reason, actor_id)
        VALUES (${withdrawal.client_id}::uuid, 'withdrawal', ${-Number(withdrawal.amount)}, ${updated.currency}, ${updated.balance}, 'Client Vault Withdrawal', ${withdrawalId}, ${note || 'Admin approved withdrawal'}, ${auth.session.user.id}::uuid)
      `
      ...
      return NextResponse.json({ success: true, status: 'approved', balance: updated.balance })
    }
