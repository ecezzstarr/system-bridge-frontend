...
export async function POST(request: NextRequest) {
  ...
  const [entry] = await sql`INSERT INTO client_vault_ledger (client_id,type,amount,currency,status,reference,note,created_by) VALUES (${clientId}::uuid,'admin_credit',${amount},${currency},'posted',${body.reference || null},${body.note || 'Administration credit'},${auth.session.user.id}::uuid) RETURNING *`
