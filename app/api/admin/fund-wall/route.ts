...
export async function GET() {
  try {
    ...
    const walletStats = await sql`
      SELECT 
        COALESCE(SUM(CAST(balance_trx AS DECIMAL)), 0) as total_balance,
    ...
    const topWallets = await sql`
      SELECT 
        ...
        COALESCE(w.balance_trx::DECIMAL, 0) as balance
      FROM users u
      LEFT JOIN wallets w ON u.id = w.user_id
      WHERE u.is_active = true
      ORDER BY w.balance_trx DESC
      LIMIT 10
    `
    ...
    const tierDistribution = await sql`
      SELECT 
        CASE 
          WHEN CAST(balance_trx AS DECIMAL) >= 25000 THEN 'diamond'
          WHEN CAST(balance_trx AS DECIMAL) >= 10000 THEN 'platinum'
          WHEN CAST(balance_trx AS DECIMAL) >= 5000 THEN 'gold'
          WHEN CAST(balance_trx AS DECIMAL) >= 1000 THEN 'silver'
          ELSE 'bronze'
        END as tier,
        COUNT(*) as count
      FROM wallets
    ...
    if (Array.isArray(tierDistribution)) {
      tierDistribution.forEach((tier: any) => {
        tierMap[tier.tier] = parseInt(tier.count)
      })
    }
