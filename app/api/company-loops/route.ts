...
export async function GET(request: NextRequest) {
  try {
    ...
    const role = requestedRole && ['client', 'agent', 'bridger', 'admin'].includes(requestedRole)
      ? requestedRole
      : null

    const rows = role
      ? await sql`
          SELECT id, loop_number, title, purpose, stage, position, functions, economics,
                 responsibilities, boundaries, agreement_version, audience, status,
                 created_at, updated_at, published_at
          FROM company_loops
          WHERE status = 'published'
            AND (${role} = ANY(audience) OR 'all' = ANY(audience))
          ORDER BY loop_number ASC, created_at ASC
        `
      : await sql`
          SELECT id, loop_number, title, purpose, stage, position, functions, economics,
                 responsibilities, boundaries, agreement_version, audience, status,
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!Number.isInteger(loopNumber) || loopNumber < 1 || !title) {
      return NextResponse.json({ error: 'loopNumber and title are required' }, { status: 400 })
    }
    ...
    const rows = await sql`
      INSERT INTO company_loops
        (loop_number, title, purpose, stage, position, functions, economics,
         responsibilities, boundaries, agreement_version, audience, status, created_by, published_at)
      VALUES
        (${loopNumber}, ${title}, ${String(body.purpose || '')}, ${String(body.stage || '')},
         ${String(body.position || 'all')}, ${String(body.functions || '')}, ${String(body.economics || '')},
         ${String(body.responsibilities || '')}, ${String(body.boundaries || '')},
         ${body.agreementVersion ? String(body.agreementVersion) : null}, ${audience},
         ${publish ? 'published' : 'draft'}, ${user.id}::uuid, ${publish ? sql`NOW()` : null})
      RETURNING *
    `
