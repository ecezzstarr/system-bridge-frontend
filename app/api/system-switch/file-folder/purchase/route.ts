...
import { ensureClientFileFolderSchema, claimFileFolder } from '@/lib/client-file-folder'
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      purchase: record,
      message: 'Payment recorded. Administration must confirm the payment before the File Folder is activated.'
    }, { status: 201 })
  } catch (error: any) {
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const folder = await claimFileFolder(sql, clientId, fileNumber, clientName)
    if (!folder) return NextResponse.json({ error: 'File Folder could not be claimed' }, { status: 409 })

    const [confirmed] = await sql`
      UPDATE file_folder_purchases SET status='confirmed', confirmed_at=NOW() WHERE id=${purchaseId}::uuid RETURNING *
    `
