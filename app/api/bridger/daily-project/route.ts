...
export async function GET(req: NextRequest) {
  try {
    ...
    const [claim] = await sql`
      SELECT c.id, c.prospect_id, c.claim_date, c.claimed_at,
             p.*
    ...
    console.error('[daily-project] GET error:', error)
    return NextResponse.json({ error: error?.message || 'Unable to load daily project' }, { status: 500 })
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Only verified WhatsApp prospects enter the free daily outreach pool.
    // A prospect can be issued only once as a daily bonus, preventing reuse
    // of the same lead across different Bridgers.
    ...
    console.error('[daily-project] POST error:', error)
    return NextResponse.json({ error: error?.message || 'Unable to claim daily project' }, { status: 500 })
  }
}
