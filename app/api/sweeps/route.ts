...
export async function GET() {
  try {
    ...
    const sweeps = await sql`
      ...
      LEFT JOIN users uw ON sa.created_by = uw.id::text
      LEFT JOIN users auw ON sa.approved_by = auw.id::text
...
export async function POST(request: Request) {
  try {
    ...
    const userResult = await sql`SELECT id, role FROM users WHERE id = ${session.user.id}::uuid`
    if (userResult[0]?.role !== 'creator') {
      return NextResponse.json({ error: 'Forbidden - Creator access required' }, { status: 403 })
    }
    ...
    const platformWallet = (wallets as any[]).find((w: any) => w.tron_address === 'TNzNPekX1tbeFYRe3DPjnNV2dG6QfvHymte')
    const companyWallet = (wallets as any[]).find((w: any) => w.tron_address === 'THGBvmPt3XEb8mbSRXViA93GkW3PpCanJk')
    ...
    if (action === 'request') {
      ...
      const result = await sql`
        ...
        VALUES (${platformWallet.id}, ${companyWallet.id}, ${amount}, 'pending', ${session.user.id}::uuid)
      ...
      const result = await sql`
        ...
        SET status = 'approved', approved_by = ${session.user.id}::uuid, approved_at = NOW()
      ...
      const result = await sql`
        UPDATE sweeps_approval 
        SET status = 'rejected', approved_by = ${session.user.id}::uuid, approved_at = NOW()
        WHERE id = ${sweepId}::uuid
        RETURNING id, status
      `
