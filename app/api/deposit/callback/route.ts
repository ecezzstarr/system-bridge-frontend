...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (type === 'file_folder') {
      ...
      if (!Number.isFinite(verifiedUsd) || Math.abs(verifiedUsd * 10 - paidTrx) > 0.000001) return NextResponse.redirect(`${baseUrl}/system-switch?error=payment_amount_mismatch`)
  ...
  } catch { return NextResponse.redirect(`${baseUrl}/system-switch?error=processing_failed`) }
}
