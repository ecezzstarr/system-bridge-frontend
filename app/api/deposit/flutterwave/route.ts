...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!Number.isFinite(amountUSD) || amountUSD < 1) return NextResponse.json({ success: false, error: 'Valid payment amount required' }, { status: 400 })
  ...
  } catch (error: any) { return NextResponse.json({ success: false, error: error.message || 'Payment initialization failed' }, { status: 500 }) }
}
