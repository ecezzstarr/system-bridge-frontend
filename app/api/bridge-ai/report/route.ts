...
async function clientIdFromRequest(request: NextRequest) {
 const token = request.headers.get('authorization')?.replace(/^Bearer\s+/i, '').trim() || null
 return resolveClientToken(token)
}

export async function POST(request: NextRequest) {
  const clientId = await clientIdFromRequest(request)
  if (!clientId) return NextResponse.json({ error: 'Authentication required' }, { status: 401 })
  try {
    const sql = neon(process.env.DATABASE_URL!)
    await ensureClientWorkshopSchema(sql)
    const [client] = await sql`SELECT id, name, file_number, referred_by_bridger_id AS assigned_bridger_id FROM users WHERE id=${clientId}::uuid LIMIT 1`
