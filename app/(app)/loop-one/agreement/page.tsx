...
const TERMS = {
  agent: {
    title: 'Agent Loop One Participation Agreement',
    role: 'Agent',
  ...
  bridger: {
    title: 'Bridger Loop One Participation Agreement',
    role: 'Bridger',
...
export default function LoopOneAgreementPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/loop-one/agreement', {
      ...
      .catch(() => {
        setError('Unable to verify the agreement status.')
        setLoading(false)
      })
  ...
  if (loading || !terms) {
    return <div className="min-h-screen bg-slate-950 text-white flex items-center justify-center">Loading Loop One agreement...</div>
  }
      ...
      <div className="mx-auto max-w-3xl">
        <div className="mb-6 rounded-2xl border border-cyan-500/30 bg-cyan-500/10 p-5">
          <div className="flex items-start gap-4">
            ...
            <div>
              <p className="text-xs uppercase tracking-[0.2em] text-cyan-400">Loop One · Required</p>
              <h1 className="mt-1 text-2xl font-bold">Your Loop One position must be established</h1>
              <p className="mt-2 text-sm leading-6 text-slate-300">Read this agreement before entering Loop One functions. Your role determines the position and participation terms shown below.</p>
            </div>
          </div>
        </div>
        ...
        <section className="rounded-2xl border border-slate-700 bg-slate-900/90 overflow-hidden">
          <header className="border-b border-slate-700 p-6 sm:p-8">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-slate-500">Weave of Presence · System Switch · Bridge Radiance</p>
                <h2 className="text-xl font-bold sm:text-2xl">{terms.title}</h2>
              </div>
            </div>
          ...
          <div className="p-6 sm:p-8 space-y-6">
            ...
            <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-5">
              <p className="text-sm font-semibold text-amber-300">Access condition</p>
              <p className="mt-2 text-sm leading-6 text-slate-400">Until this agreement is signed, Loop One activation and Loop One earning functions remain unavailable for this {terms.role} position.</p>
            </div>

            {error && <div className="rounded-lg border border-red-500/30 bg-red-500/10 p-3 text-sm text-red-300">{error}</div>}

            <label className="flex items-start gap-3 rounded-xl border border-slate-700 bg-slate-950/60 p-4">
              <CheckCircle2 className="mt-0.5 h-5 w-5 text-cyan-400 shrink-0" />
            ...
            <Button onClick={signAgreement} disabled={submitting} className="w-full bg-cyan-600 hover:bg-cyan-700 py-6 text-base font-semibold">
              {submitting ? 'Recording agreement...' : 'Read & Sign Loop One Agreement'}
            </Button>
          </div>
        </section>
      </div>
