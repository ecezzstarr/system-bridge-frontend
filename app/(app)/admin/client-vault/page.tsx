...
export default function AdminClientVaultPage() {
  ...
  const credit = async () => {
    ...
    const res = await fetch('/api/admin/client-vault/credit', {
      ...
      body: JSON.stringify({ client_id: selected.id, amount: Number(amount), reference: 'Admin Client Vault', note: reason || 'Administration Flame Coin credit' }),
    })
    const data = await res.json()
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="grid gap-6 lg:grid-cols-[1.3fr_.7fr]">
          <section className="rounded-xl border border-slate-800 bg-slate-900/70 overflow-hidden">
            ...
            <div className="divide-y divide-slate-800">{clients.map(c => <button key={c.id} onClick={() => setSelected(c)} className={`w-full p-4 text-left hover:bg-slate-800/60 ${selected?.id === c.id ? 'bg-slate-800/60' : ''}`}><div className="flex items-center justify-between gap-4"><div><p className="font-semibold">{c.name}</p><p className="text-xs text-slate-500">{c.file_number || 'No File Number'} · {c.email}</p></div><div className="text-right space-y-1"><p className="text-sm font-bold text-emerald-400">Vault · {c.vault_balance} Flame Coin</p><p className="text-[10px] text-violet-300">Siblings Funds · {c.sibling_flame_coin} Flame Coin</p><p className="text-[10px] text-sky-300">Main Wallet · {c.main_flame_coin} Flame Coin</p></div></div></button>)}</div>
          </section>
          ...
          <section className="rounded-xl border border-slate-800 bg-slate-900/70 p-5">
            ...
            {selected ? <div className="space-y-3"><div className="rounded-lg bg-slate-950 p-4"><p className="text-xs text-slate-500">Client Vault · Administration Flame Coin</p><p className="text-3xl font-bold text-emerald-400">{selected.vault_balance} Flame Coin</p><div className="mt-3 grid grid-cols-2 gap-2 text-[10px]"><div className="rounded-lg border border-violet-400/10 bg-violet-400/5 p-2 text-violet-300">Siblings Funds<br/><span className="text-sm font-bold">{selected.sibling_flame_coin}</span> Flame Coin</div><div className="rounded-lg border border-sky-400/10 bg-sky-400/5 p-2 text-sky-300">Main Client Wallet<br/><span className="text-sm font-bold">{selected.main_flame_coin}</span> Flame Coin</div></div></div><input value={amount} onChange={e => setAmount(e.target.value)} placeholder="Credit amount" type="number" min="0" step="any" className="w-full rounded-lg border border-slate-700 bg-slate-950 p-3 text-sm" /><input value={reason} onChange={e => setReason(e.target.value)} placeholder="Reason / reference" className="w-full rounded-lg border border-slate-700 bg-slate-950 p-3 text-sm" /><button onClick={credit} disabled={!amount} className="w-full rounded-lg bg-emerald-600 py-3 font-semibold disabled:opacity-50">Credit Flame Coin to Client Vault</button><p className="text-xs leading-5 text-slate-500">Administration credits the Client Vault only. Siblings Funds and the Main Client Wallet remain separate balances.</p></div> : <p className="text-sm text-slate-500">Select a Client to operate the Vault.</p>}
          </section>
        </div>
