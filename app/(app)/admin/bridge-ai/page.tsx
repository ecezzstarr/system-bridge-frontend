...
export default function AdminBridgeAIPage() {
  ...
  const load=async()=>{
    ...
    const [r,c]=await Promise.all([fetch('/api/admin/bridge-ai-reports',{headers}),fetch('/api/admin/client-workshop',{headers})])
    const rd=await r.json(), cd=await c.json()
    setReports(rd.reports||[]); setClients(cd.clients||[]); setAgents(cd.agents||[])
  }
