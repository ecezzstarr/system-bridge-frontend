...
function AgentBridgerCount() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (response.ok) {
          setCount(Number(data.activeCount ?? data.count ?? 0))
          setTarget(Number(data.target ?? 6))
        }
      } catch {
        setCount(0)
      }
  ...
  return <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-4"><p className="mb-1 text-xs text-slate-500">Active Bridgers</p><p className="text-2xl font-bold text-cyan-400">{count === null ? '—' : `${count}/${target}+`}</p><p className="mt-1 text-[11px] text-slate-500">Daily operating target</p></div>
}
...
function MyBridgers() {
  ...
  useEffect(() => { const fetchBridgers = async () => { try { const response = await fetch('/api/agent/bridgers', { headers: getAuthHeaders() }); const data = await response.json(); setBridgers(data.bridgers || []) } catch (error) { console.error('Error fetching bridgers:', error) } finally { setLoading(false) } }; fetchBridgers() }, [])
  return <div className="group relative"><div className="absolute -inset-0.5 bg-gradient-to-r from-blue-500 to-cyan-500 rounded-xl opacity-20 blur group-hover:opacity-40 transition duration-500"></div><div className="relative bg-slate-900/80 backdrop-blur-xl border border-slate-700 rounded-xl p-6"><div className="flex items-center justify-between mb-6"><div className="flex items-center gap-3"><Users className="h-8 w-8 text-cyan-400"/><div><h2 className="text-2xl font-bold text-white">My Bridgers</h2><p className="text-slate-400 text-sm">Manage your active Bridgers</p></div></div></div>{loading ? <p className="text-slate-400 text-center py-8">Loading bridgers...</p> : bridgers.length === 0 ? <div className="text-center py-12"><Users className="h-16 w-16 text-slate-600 mx-auto mb-4"/><p className="text-slate-400 mb-2">No active Bridgers assigned yet</p><p className="text-sm text-slate-500">Build your Bridger team to reach the daily operating target.</p></div> : <div className="grid grid-cols-1 md:grid-cols-3 gap-4">{bridgers.map((bridger) => <div key={bridger.id} className="bg-slate-800/50 border border-slate-700 rounded-lg p-4"><div className="flex items-center gap-3 mb-3"><div className="w-12 h-12 rounded-full bg-emerald-500/20 flex items-center justify-center text-emerald-400 text-xl font-bold">{bridger.name?.charAt(0) || 'B'}</div><div><h3 className="text-white font-semibold">{bridger.name}</h3><p className="text-xs text-slate-400">{bridger.email}</p></div></div><div className="grid grid-cols-2 gap-2 text-center mb-3"><div className="bg-slate-900/50 rounded p-2"><p className="text-xs text-slate-500">Clients</p><p className="text-lg font-bold text-cyan-400">{bridger.clientCount}</p></div><div className="bg-slate-900/50 rounded p-2"><p className="text-xs text-slate-500">Earnings</p><p className="text-lg font-bold text-emerald-400">{bridger.earnings} TRX</p></div></div><Button variant="outline" className="w-full border-slate-600 text-sm">View Details</Button></div>)}</div>}</div></div>
}
